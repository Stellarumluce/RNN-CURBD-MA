# -*- coding: utf-8 -*-
"""
Created on Fri Jul 12 18:02:38 2024
...
vectorized_retinal_data = resized_retinal_data.reshape(new_size[0] * new_size[1], T_resized)
...
p = 1  # Sparsity
J_d, u, P, x_0 = initialize_variables(N, g, p, alpha)
...
for iter in range(1):
    ...
    for neuron_num in range(N):
        ...
        for t in range(1, len(t_vec)):
            x_d[:, t] = (1 - dt / tau) * x_d[:, t-1] + (dt / tau) * (np.dot(J_d, r_d) + 2*u * resized_exc_data[neuron_num, t-1] )
...
error_avg = np.mean(np.abs(final_zpt - resized_exc_data))
...
for iter in range(T_resized):
    plt.figure()
    plt.subplot(1, 2, 1)
    img1 = plt.imshow(final_zpt[:, iter].reshape(new_size), aspect='auto')
    plt.title('Output Frame')
    plt.colorbar(img1)

    plt.subplot(1, 2, 2)
    img2 = plt.imshow(resized_exc_data[:, iter].reshape(new_size), aspect='auto')
