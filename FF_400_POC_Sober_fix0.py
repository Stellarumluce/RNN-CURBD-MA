...
for iter in range(1):
    ...
    for neuron_num in range(N):
        ...
        for t in range(1, len(t_vec)):
            x_d[:, t] = (1 - dt / tau) * x_d[:, t-1] + (dt / tau) * (np.dot(J_d, r_d) + u * resized_exc_data[neuron_num, t-1] + u*resized_exc_data[neuron_num, t-1])
            x[:, t] = (1 - dt / tau) * x[:, t-1] + (dt / tau) * (np.dot(J_N[:, :, neuron_num], r) + u * resized_exc_data[neuron_num, t-1])
            r_d = np.tanh(x_d[:, t])
            r = np.tanh(x[:, t])
            
            e = np.dot(J_N[:, :, neuron_num], r) - np.dot(J_d, r_d) - u * resized_exc_data[neuron_num, t-1]
            k = np.dot(P, r)
            rPr = np.dot(r.T, k)
            c = 1.0 / (1.0 + rPr)
...
for neuron_num in range(N):
    ...
    for ti in range(simtime_len):
        x_test = (1 - dt / tau) * x_test + (dt / tau) * (np.dot(J_pre_N[:, :, neuron_num], r_test) + u *resized_exc_data[neuron_num, ti-1])
        r_test = np.tanh(x_test)
...
for iter in range(1):
    ...
    for neuron_num in range(N):
        ...
        for t in range(1, len(t_vec)):
            x_d[:, t] = (1 - dt / tau) * x_d[:, t-1] + (dt / tau) * (np.dot(J_d, r_d) + u * normalized_exc_data[neuron_num, t-1] + u_in * vectorized_retinal_data[neuron_num, t-1])
            x[:, t] = (1 - dt / tau) * x[:, t-1] + (dt / tau) * (np.dot(J_N[:, :, neuron_num], r)+ u_in * vectorized_retinal_data[neuron_num, t-1])
            r_d = np.tanh(x_d[:, t])
            r = np.tanh(x[:, t])
...
for neuron_num in range(N):
    ...
    r_test = np.tanh(x_test)
    for ti in range(simtime_len):
        x_test = (1 - dt / tau) * x_test + (dt / tau) * (np.dot(J_pre_N[:, :, neuron_num], r_test) + u_in * vectorized_retinal_data[neuron_num, ti-1])
        r_test = np.tanh(x_test)
        z = np.dot(W_pre[:, neuron_num].T, r_test)  # Use the final trained weights
        zpt[ti] = z
    final_zpt[neuron_num, :] = zpt.T
