...
for t in range(1, len(t_vec)):
    # Update neuron state
    x[:, t] = (1 - dt / tau) * x[:, t-1] + (dt / tau) * (np.dot(J, r)) + 0.1 * u * vectorized_retinal_data[:, t-1]
...
for ti in range(simtime_len):
    x_test = (1.0 - dt) * x_test + np.dot(J, r_test * dt) + 0.1 * u * vectorized_retinal_data[:, ti-1]
    r_test = np.tanh(x_test)
