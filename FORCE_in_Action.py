# -*- coding: utf-8 -*-
"""
Created on Tue Sep 17 13:01:38 2024
...
new_dt = 1    # New temporal resolution is 10 ms
...
for iter in range(len(t_vec)):
    # Plot the results
    plt.figure()
    plt.subplot(1, 2, 1)
    plt.imshow(final_zpt[:, iter].reshape(new_size))
    plt.title('Output Frame')
    plt.subplot(1, 2, 2)
    plt.imshow(normalized_exc_data[:, iter].reshape(new_size))
