# -*- coding: utf-8 -*-
"""
Created on Fri May 24 00:20:51 2024
...
def train_network(N, T, g, dt, a, num_iterations):
    ...
    for iteration in range(num_iterations):
        ...
        print(f"Mean target data (a): {np.mean(a)}")
        print(f"Explained variance: {explained_var}")
        print(f"Total variance: {total_var}")
        
        if total_var == 0:
            pvar = 0  # Avoid division by zero
        else:
...
# Use a sinusoidal target data with noise
a = np.sin(2 * np.pi * frequency * time_steps)  # Sinusoidal target data
noise = 0.1 * np.random.randn(N, len(time_steps))  # Add noise to increase variance
a = np.tile(a, (N, 1)) + noise  # Combine sinusoid with noise
a = np.clip(a, -1, 1)  # Ensure the values are within [-1, 1]
...
plt.xlabel('Iteration')
plt.ylabel('Total Variance (Total_Var)')
plt.title('Training Progress')
plt.figure(figsize=(10, 6))
plt.plot(range(1, num_iterations + 1), Explained_var, marker='o')
plt.xlabel('Iteration')
plt.ylabel('Explained Variance (Explained_var)')
plt.title('Training Progress')
plt.grid(True)
plt.figure(figsize=(10, 6))
