# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 18:54:02 2024
...
output_dir = os.path.expanduser('~/NAS/projects/2024VisDev/FF_plots_400_Synthetic_POC')
...
a_album = np.zeros((40, 40, T)) 
for t in range(T):
    a_album[:,:,t] = a[:, t].reshape(40, 40)
resized_a_data = np.zeros((new_size[0], new_size[1], T))
# for idx in range(T):
#     plt.figure()
#     plt.imshow(resized_retinal_data[:, :,idx])
for t in range(T):
    resized_frame = resize(a_album[:, :, t], new_size, anti_aliasing=True, mode='reflect')
    resized_a_data[:, :, t] = resized_frame
a = resized_a_data.reshape(new_size[0] * new_size[1], T)
# Parameters
N = a.shape[0]
...
for iter in range(T):
    ...
    plt.figure()
    plt.subplot(1, 2, 1)
    img1 = plt.imshow(final_zpt[:, iter].reshape(20, 20), aspect='auto')
    plt.title(f'Output Frame {iter}')
    plt.colorbar(img1)

    plt.subplot(1, 2, 2)
    img2 = plt.imshow(a[:, iter].reshape(20, 20), aspect='auto')
