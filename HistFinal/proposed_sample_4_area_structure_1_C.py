# -*- coding: utf-8 -*-
"""
Created on Tue Sep 24 13:24:25 2024
...
C_mid = np.random.randn(N, N) / np.sqrt(N)
# Training Loop only time loop
for t in range(1, len(t_vec)):
    # Update neuron state
    x_1[:, t] = (1 - dt / tau) * x_1[:, t-1] + (dt / tau) * (np.dot(J_1, r_1)) + np.dot(u, vectorized_retinal_data[:, t-1])
    # Update firing rate
    r_1 = np.tanh(x_1[:, t])  # Ensure r is (N,)
    y_input_1 = np.dot(C1, r_1)
    
    x_mid[:, t] = (1 - dt / tau) * x_mid[:, t-1] + (dt / tau) * (np.dot(J_mid, r_mid)) + np.dot(u , y_input_1)
    # Update firing rate
    r_mid = np.tanh(x_mid[:, t])  # Ensure r is (N,)
    y_input_mid = np.dot(C_mid, r_mid)
    
    
    x_2[:, t] = (1 - dt / tau) * x_2[:, t-1] + (dt / tau) * (np.dot(J_2, r_2)) + np.dot(u , y_input_1)
    # Update firing rate
    ...
    x_3[:, t] = (1 - dt / tau) * x_3[:, t-1] + (dt / tau) * (np.dot(J_3, r_3)) + np.dot(u , y_input_2) + np.dot(u , y_input_mid)
    # Update firing rate
    ...
    k_mid = np.dot(P_mid, r_mid)
    rPr_mid = np.dot(r_mid.T, k_mid)
    c_mid = 1.0 / (1.0 + rPr_mid)
    # C2 -=  c2 * np.outer(k2, e_minus)
    J_mid = J_mid - np.outer(e_minus, k_mid)*c_mid
    P_mid = P_mid - c_mid * np.outer(k_mid, k_mid)
    ####
...
r_test_mid = np.tanh(x_test_mid)
z = z_0
for ti in range(simtime_len):
    x_test_1 = (1.0 - dt) * x_test_1 + np.dot(J_1, r_test_1 * dt)  + np.dot(u, vectorized_retinal_data[:, ti-1])
    r_test_1 = np.tanh(x_test_1)
    y_test_input_1 = np.dot(C1, r_test_1)
    x_test_mid = (1.0 - dt) * x_test_mid + np.dot(J_mid, r_test_mid * dt)  + np.dot(u , y_test_input_1)
    r_test_mid = np.tanh(x_test_mid)
    y_test_input_mid = np.dot(C_mid, r_test_mid)
    x_test_2 = (1.0 - dt) * x_test_2 + np.dot(J_2, r_test_2 * dt) +  np.dot(u , y_test_input_1)
    r_test_2 = np.tanh(x_test_2)
    y_test_input_2 = np.dot(C2, r_test_2)
    x_test_3 = (1.0 - dt) * x_test_3+ np.dot(J_3, r_test_3 * dt) +  np.dot(u , y_test_input_2) + np.dot(u , y_test_input_mid)
    r_test_3 = np.tanh(x_test_3)
...
for iter in range(simtime_len):
    ...
    # Plot the first image (output frame)
    im1 = axes[0].imshow(zpt[:, iter].reshape(new_size), cmap='viridis')
    axes[0].set_title(f'Output Frame: {iter}')
    cbar1 = plt.colorbar(im1, ax=axes[0], orientation='vertical')
    
    # Plot the second image (target frame)
    im2 = axes[1].imshow(a[:, iter].reshape(new_size), cmap='viridis')
